...
var Obsidian = {
  ...
  subtocSpy: function (offset) {
    var tocContainer = $("#sub-toc");
    var toc = tocContainer,
      tocHeight = toc.height();
    scrollSpy(tocContainer, {
      offset: 200
...
$(document).ready(function () {
  ...
  $('body').on('click', function (e) {
    ...
    if (!tag && !rel) return;
    switch (true) {
      case (tag.indexOf('sub-switchmenu') != -1):
        // 注释掉：文章右上角的分享功能
        ...
        // }
        // 改成弹出目录sidebar
        var sub_switchMenu = $('.sub-switchmenu');
        var sub_side_bar = $('.sub-animate');
        if (sub_side_bar.hasClass('fadeOutRight')) {
          sub_switchMenu.removeClass('icon-menu').addClass('icon-cross');
          sub_side_bar.removeClass('fadeOutRight').addClass('fadeInRight');
        } else {
          sub_switchMenu.removeClass('icon-cross').addClass('icon-menu');
          sub_side_bar.removeClass('fadeInRight').addClass('fadeOutRight');
        }
